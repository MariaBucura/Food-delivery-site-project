...
<div class="account-area">
    <div class="user-settings-tab">
        <a  href="displayorders.php">Order history</a>
        <a  href="#">Account settings</a>
        <form method="POST" action="delete.php" class="delete-account">
        <button type="submit" name="submit" id="submit" class="logout-button" style="font-size: 30px; padding: 0">Delete account</button>
        </form>
    </div>
    <div class="user-info">
        <div class="user-display-name"><?php echo $_SESSION['username']; ?>'s Page</div>
        <div class="user-settings">
            <div class="email-form">
            <form method="POST" action="updateemail.php" class="add-restaurant-container">
                <h2>Change email</h2>
                <?php
                if(isset($_SESSION['message'])){
                    ?>
                    <p style="color: pink"><?php echo $_SESSION['message']; ?></p>
                    <?php
                    unset($_SESSION['message']);
                }
                ?>
                <label for="email">Email</label>
                <input class="form-input" type="text" placeholder="Enter new email" name="email" id="email" required><br>
                <button type="submit" name="submit" id="submit" class="nav-button">Change email</button><br>
            </form>
            </div>
            <div class="password-form">
            <form method="POST" action="updatepassword.php" class="add-restaurant-container">
                <h2>Change password</h2>
                <?php
                if(isset($_SESSION['password-message'])){
                    ?>
                    <p style="color: pink"><?php echo $_SESSION['password-message']; ?></p>
                    <?php
                    unset($_SESSION['password-message']);
                }
                ?>
                <label for="old-password">Old password</label>
                <input class="form-input" type="password" placeholder="Enter old password" name="old-password" id="old-password" required><br>
                <label for="new-password">New password</label>
                <input class="form-input" type="password" placeholder="Enter new password" name="new-password" id="new-password" required><br>
                <button type="submit" name="submit" id="submit" class="nav-button">Change Password</button><br>
            </form>
            </div>
        </div>
    </div>
